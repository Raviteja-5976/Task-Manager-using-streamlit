...
from mlauth import fetch_users
...
def min():
    ...
    st.title("GDSC MACHINE LEARNING DOMAIN")
...
def task():
    ...
    member_names_query = """
        SELECT m_id, name FROM members where db_name= "Machine Learning Team"
        """
    ...
    if st.button("Add Task"):
        ...
        ts_values = (selected_m_id, "Machine Learning Team", name_t, assn_on, status, com_on, comment)
...
def stat():
    ...
    with tab1:
        ...
        FROM task
        WHERE status = "To-Do" AND do_name = "Machine Learning Team"
    ...
    with tab2: 
        ...
        SELECT member_id, name_t, assig_on, status, comp_on, comment
        FROM task
        WHERE status = "Progressing" AND do_name = "Machine Learning Team"
        ORDER BY assig_on DESC
        '''
    ...
    with tab3:
        ...
        SELECT member_id, name_t, assig_on, status, comp_on, comment
        FROM task
        WHERE status = "Completed" AND do_name = "Machine Learning Team"
        ORDER BY assig_on DESC
        '''
